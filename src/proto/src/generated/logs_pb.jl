...
function meta(::Type{LogRecord})
    ProtoBuf.metalock() do
        if !isassigned(__meta_LogRecord)
            ...
            wtype = Dict(:time_unix_nano => :fixed64, :observed_time_unix_nano => :fixed64, :flags => :fixed32)
            allflds = Pair{Symbol,Union{Type,String}}[:time_unix_nano => UInt64, :observed_time_unix_nano => UInt64, :severity_number => Int32, :severity_text => AbstractString, :name => AbstractString, :body => opentelemetry.proto.common.v1.AnyValue, :attributes => Base.Vector{opentelemetry.proto.common.v1.KeyValue}, :dropped_attributes_count => UInt32, :flags => UInt32, :trace_id => Vector{UInt8}, :span_id => Vector{UInt8}]
            meta(target, LogRecord, allflds, ProtoBuf.DEF_REQ, fnum, ProtoBuf.DEF_VAL, ProtoBuf.DEF_PACK, wtype, ProtoBuf.DEF_ONEOFS, ProtoBuf.DEF_ONEOF_NAMES)
        end
        __meta_LogRecord[]
    end
end
...
function meta(::Type{InstrumentationLibraryLogs})
    ProtoBuf.metalock() do
        if !isassigned(__meta_InstrumentationLibraryLogs)
            __meta_InstrumentationLibraryLogs[] = target = ProtoMeta(InstrumentationLibraryLogs)
            allflds = Pair{Symbol,Union{Type,String}}[:instrumentation_library => opentelemetry.proto.common.v1.InstrumentationLibrary, :log_records => Base.Vector{LogRecord}, :schema_url => AbstractString]
            meta(target, InstrumentationLibraryLogs, allflds, ProtoBuf.DEF_REQ, ProtoBuf.DEF_FNUM, ProtoBuf.DEF_VAL, ProtoBuf.DEF_PACK, ProtoBuf.DEF_WTYPES, ProtoBuf.DEF_ONEOFS, ProtoBuf.DEF_ONEOF_NAMES)
        end
...
function meta(::Type{ResourceLogs})
    ProtoBuf.metalock() do
        if !isassigned(__meta_ResourceLogs)
            ...
            allflds = Pair{Symbol,Union{Type,String}}[:resource => opentelemetry.proto.resource.v1.Resource, :instrumentation_library_logs => Base.Vector{InstrumentationLibraryLogs}, :schema_url => AbstractString]
            meta(target, ResourceLogs, allflds, ProtoBuf.DEF_REQ, ProtoBuf.DEF_FNUM, ProtoBuf.DEF_VAL, ProtoBuf.DEF_PACK, ProtoBuf.DEF_WTYPES, ProtoBuf.DEF_ONEOFS, ProtoBuf.DEF_ONEOF_NAMES)
        end
        __meta_ResourceLogs[]
    end
end
function Base.getproperty(obj::ResourceLogs, name::Symbol)
    ...
    elseif name === :instrumentation_library_logs
        return (obj.__protobuf_jl_internal_values[name])::Base.Vector{InstrumentationLibraryLogs}
    elseif name === :schema_url
        return (obj.__protobuf_jl_internal_values[name])::AbstractString
    else
        getfield(obj, name)
...
function meta(::Type{LogsData})
    ProtoBuf.metalock() do
        if !isassigned(__meta_LogsData)
            ...
            allflds = Pair{Symbol,Union{Type,String}}[:resource_logs => Base.Vector{ResourceLogs}]
            meta(target, LogsData, allflds, ProtoBuf.DEF_REQ, ProtoBuf.DEF_FNUM, ProtoBuf.DEF_VAL, ProtoBuf.DEF_PACK, ProtoBuf.DEF_WTYPES, ProtoBuf.DEF_ONEOFS, ProtoBuf.DEF_ONEOF_NAMES)
        end
        __meta_LogsData[]
    end
end
...
export SeverityNumber, LogRecordFlags, LogsData, ResourceLogs, InstrumentationLibraryLogs, LogRecord
